...
function TreeSet({ svgH }) {
  return (
    <Svg width={TOTAL_W} height={svgH}>
      <Defs>
        {TREES.map((t, idx) => (
          <RadialGradient key={`rg${idx}`} id={`rg${idx}`} cx="50%" cy="40%" r="60%">
      ...
      {TREES.map((t, idx) => {
        ...
        return (
          <G key={idx}>
            <Rect x={tx} y={base - t.trunkH} width={t.trunkW} height={t.trunkH} fill={t.vine} rx={t.trunkW * 0.25} />
            <Rect x={tx + t.trunkW * 0.35} y={base - t.trunkH} width={t.trunkW * 0.12} height={t.trunkH} fill="rgba(0,0,0,0.18)" rx={2} />

            {t.vines.map((v, vi) => (
              <Path
                key={vi}
                d={`M ${bx + v.x} ${base - t.trunkH * 0.7} Q ${bx + v.x + v.sway} ${base - t.trunkH * 0.35} ${bx + v.x + v.sway * 0.4} ${base - t.trunkH * 0.7 + v.len}`}
                fill="none" stroke={t.vine} strokeWidth={v.thick} strokeLinecap="round" opacity="0.7"
              />
            ))}
            ...
            {t.leaves.map((lf, li) => (
              <Ellipse
                key={li}
                cx={bx + lf.cx} cy={top + t.h * 0.1 + lf.cy}
                rx={lf.rx} ry={lf.ry}
                fill={lf.col} opacity={lf.opacity}
                transform={`rotate(${lf.rot}, ${bx + lf.cx}, ${top + lf.cy})`}
              />
            ))}

            <Ellipse
              cx={bx + t.w / 2} cy={top + t.h * 0.05}
              rx={t.w * 0.55} ry={t.h * 0.18}
              fill={`url(#rg${idx})`} opacity="0.88"
            />
            ...
            {t.hasFlower && (
              <Circle
                ...
                cy={top + t.h * 0.08}
                r={5} fill={t.flower} opacity="0.92"
              />
            )}

            <Path d={`M ${tx} ${base} Q ${tx - 8} ${base - 12} ${tx - 4} ${base - t.trunkH * 0.08}`} fill={t.vine} opacity="0.55" />
            <Path d={`M ${tx + t.trunkW} ${base} Q ${tx + t.trunkW + 8} ${base - 12} ${tx + t.trunkW + 4} ${base - t.trunkH * 0.08}`} fill={t.vine} opacity="0.55" />
          </G>
        );
      })}
    </Svg>
...
const FALL_DATA = Array.from({ length: N_FALLING }, (_, i) => {
  ...
  return {
    id: i, startX: r() * SW, size: 8 + r() * 18,
    duration: 5000 + r() * 8000, delay: r() * 12000,
    sway: (r() - 0.5) * SW * 0.4,
    color: LEAF_COLORS[Math.floor(r() * LEAF_COLORS.length)],
    rotEnd: 180 + r() * 360, opacity: 0.55 + r() * 0.45,
  };
});
...
function FallingLeaf({ startX, size, duration, delay, sway, color, rotEnd, opacity }) {
  ...
  useEffect(() => {
    const run = () => {
      ty.setValue(-size); tx.setValue(0); rot.setValue(0); op.setValue(opacity);
      Animated.parallel([
  ...
  return (
    <Animated.View pointerEvents="none" style={{
      ...
      width: size, height: size * 0.55, borderRadius: size * 0.5,
...
function MistLayer({ yFrac, opacity, speed }) {
  ...
  return (
    <Animated.View pointerEvents="none" style={{
      position: 'absolute', top: SH * yFrac, left: 0,
      width: SW * 2, height: 40 + opacity * 30,
      backgroundColor: 'rgba(180, 255, 180, 0.055)',
      borderRadius: 20, opacity,
...
const FIREFLIES = Array.from({ length: 18 }, (_, i) => {
  ...
  return { id: i, x: r() * SW, y: SH * (0.3 + r() * 0.6), dur: 800 + r() * 1400, delay: r() * 6000, size: 2 + r() * 3 };
});
...
function Firefly({ x, y, dur, delay, size }) {
  ...
  return (
    <Animated.View pointerEvents="none" style={{
      ...
      backgroundColor: '#aaff44', opacity: op,
      shadowColor: '#aaff44', shadowOpacity: 1, shadowRadius: size * 2, shadowOffset: { width: 0, height: 0 },
...
export default function JungleBackground() {
  ...
  useEffect(() => {
    Animated.loop(
      Animated.timing(scrollX, { toValue: -TOTAL_W, duration: SCROLL_MS, easing: Easing.linear, useNativeDriver: true })
    ).start();
...
const styles = StyleSheet.create({
  container: { ...StyleSheet.absoluteFillObject, backgroundColor: '#04180a', overflow: 'hidden' },
  sky:       { position: 'absolute', top: 0, left: 0, right: 0, height: SH * 0.45, backgroundColor: '#061a08' },
  skyMid:    { position: 'absolute', top: SH * 0.25, left: 0, right: 0, height: SH * 0.35, backgroundColor: '#082210', opacity: 0.7 },
  skyBottom: { position: 'absolute', top: SH * 0.5, left: 0, right: 0, height: SH * 0.5, backgroundColor: '#041208', opacity: 0.8 },
  treeRow:   { position: 'absolute', bottom: GROUND_H, flexDirection: 'row', alignItems: 'flex-end' },
  ground:    { position: 'absolute', bottom: 0, left: 0, right: 0, height: GROUND_H, backgroundColor: '#061a06' },
  groundGrass: { position: 'absolute', top: 0, left: 0, right: 0, height: 4, backgroundColor: '#1a6b1a', opacity: 0.9 },
  groundDark:  { position: 'absolute', top: 4, left: 0, right: 0, height: GROUND_H - 4, backgroundColor: '#030e03' },
  overlay:   { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(2, 10, 3, 0.22)' },
});
